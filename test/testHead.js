...
describe('head', () => {
  it('Should return the same line when 1 line is given', () => {
    ...
    assert.strictEqual(head(text, { separator: '\n', count: 10 }), text);
  });
  ...
  it('Should return one character from the text', () => {
    let text = 'l1\nl2';
    assert.strictEqual(head(text, { separator: '', count: 1 }), 'l');
    text = 'This is line';
    assert.strictEqual(head(text, { separator: '', count: 1 }), 'T');
  });

  it('Should return 2 characters from the text', () => {
    const text = 'This is line';
    assert.strictEqual(head(text, { separator: '', count: 2 }), 'Th');
  });

  it('Should return given number of characters from the text', () => {
    const text = 'line1\nline2';
    assert.strictEqual(head(text, { separator: '', count: 7 }), 'line1\nl');
  });

  it('Should return all characters when count exceeds total', () => {
    const text = 'line';
    assert.strictEqual(head(text, { separator: '', count: 6 }), 'line');
  });
